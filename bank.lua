...
bot.sendText(msg.chat_id,msg.id, "• مسحت لعبه النوادي ↫","md",true)
end
end
...
if text == "بنكي" or text == "البنك" or text == "بنك" or text == "اوامر البنك" then
...
local selva = "*• اهلا بك يا ⦉ "..Text.." ⦊\n• في قائمه لعبه البنك*"
local Timo = bot.getUser(Sudo_Id) 
keyboard = {} 
keyboard.inline_keyboard = {
{
{text = ' • اوامر البنك •', callback_data = msg.sender_id.user_id.."/GPQPK"},
},
{
{text = '• اوامر الزواج •', callback_data = msg.sender_id.user_id.."/YUKDO"},
},
{
{text = '• اضغط لانشاء حساب بنكي •', callback_data=senderr..'/bankopi'},
},
{
...
local t = '*〈 اضغط علي الزر لانشاء حساب 🥺♥ 〉   *'
local m = "https://files.catbox.moe/rz15yy.jpg" 
keyboard = {} 
keyboard.inline_keyboard = {
{
{text = '• اضغط لانشاء حساب بنكي •', callback_data=senderr..'/bankopi'},
...
return send(msg.chat_id,msg.id, "*• معندكش فلوس انت مفلس 😂❤️ \n√*","md",true)
end
local convert_mony = string.format("%.0f",ballancee)
send(msg.chat_id,msg.id, "*• عدد فلوسك ينجم *⦉ `"..convert_mony.."` ⦊* جنيه 🥹❤️*","md",true)
else
send(msg.chat_id,msg.id, "*• معندكش حساب بنكي 😔 \n• ارسل ⦉ انشاء حساب بنكي ⦊\n• لفتح حساب في البنك*","md",true)
end
end
...
send(msg.chat_id,msg.id, "*• فلوسه*⦉ `"..convert_mony.."` ⦊*جنيه ❤.\n• عطلاق شكلو نصاب اصلا*","md",true)
else
send(msg.chat_id,msg.id, "*معندوش حساب بنكي ينجم 🥹❤️* ","md",true)
end
end
...
if UserInfo and UserInfo.type and UserInfo.type.luatele == "userTypeBot" then
send(msg.chat_id,msg.id,"\n• الخالدي ماعندها حساب بالبنك 🤣","md",true)  
return false
end
...
local timo_selva = bot.replyMarkup{
type = 'inline',
data = {
{
{text = news, url='http://t.me/'..bain.username..' '},
},
}
}
send(msg.chat_id,msg.id, "*🧑‍🦳 الاسم ⋮ *⦉ "..news.." ⦊\n*🆔 الحساب ⋮ *⦉ `"..cccc.."` ⦊\n*🏦 بنك ⋮ ⦉ الخالدي ⦊*\n*⚡️نوع ⋮ *⦉ "..uuuu.." ⦊\n*💰 الرصيد ⋮ *⦉ "..convert_mony.." ⦊*جنيه *\n*🥷 الزرف ⋮* ⦉ "..math.floor(pppp).." ⦊*جنيه*\n*🏆 شخصيتك ⋮ *⦉ "..shkseemg.." ⦊\n","md",false, false, false, false, timo_selva)
else
send(msg.chat_id,msg.id, "*• معندكش حساب بنكي 😔 \n• ارسل ⦉ انشاء حساب بنكي ⦊\n• لفتح حساب في البنك*","md",true)
end
end
...
bot.sendText(msg.chat_id,msg.id,"\n*← الخالدي معندهوش حساب بالبنك 🤣*","md",true)  
return false
end
...
local code = ""
for j = 1, 10 do
code = code .. list[math.random(#list)]
end
Redis:sadd(Fast.."Akshtd:Games:", code)
end
end
...
if Redis:sismember(Fast.."booob", msg.sender_id.user_id) then
local cooldown = Redis:ttl(Fast.."AkshtCooldown:"..msg.sender_id.user_id)
if cooldown > 0 then
local hours = math.floor(cooldown / 3600)
local mins = math.floor((cooldown % 3600) / 60)
local secs = cooldown % 60
return bot.sendText(msg.chat_id, msg.id, "*• مينفعش تكشط تاني دلوقتي\n• المتبقي: "..hours.." ساعة و "..mins.." دقيقة و "..secs.." ثانية*", "md", true)
end
if not Redis:sismember(Fast.."Akshtd:Games:", TextAksht) then
return bot.sendText(msg.chat_id, msg.id, "*• الرمز مستخدم قبل !*", "md", true)
end
local list = {"10000", "20000", "30000", "40000", "50000", "60000", 70000, 80000, 90000, 100000, 200000}
...
local cobonplus = tonumber(ballancee) + Number
Redis:set(Fast.."boob"..msg.sender_id.user_id, cobonplus)
Redis:setex(Fast.."AkshtCooldown:"..msg.sender_id.user_id, 18000, true)
local UserInfo = bot.getUser(msg.sender_id.user_id)
local sender_name = UserInfo.first_name or "مستخدم"
local sender_username = UserInfo.username
local sender_link = sender_username and "https://t.me/"..sender_username or "tg://user?id="..msg.sender_id.user_id
local show_name = "["..sender_name.."]("..sender_link..")"
local Info_Chats = bot.getSupergroupFullInfo(msg.chat_id)
local group_link = Info_Chats.invite_link and Info_Chats.invite_link.invite_link or "لا يوجد"
bot.sendText(msg.chat_id, msg.id, show_name.."\n\n*← مبروك احلي هدية ليك يبنلقمر 🌝 : "..Number.." جنيه 💵*\n√", "md", true)
bot.sendText("-1002611811913", 0,
"*📦 كشط كوبون جديد*\n\n"..
"*• الاسم :* "..show_name.."\n"..
"*• الايدي :* "..msg.sender_id.user_id.."\n"..
"*• قيمة الكوبون :* "..Number.." جنيه 💵\n"..
"*• رابط الجروب :* "..group_link, "md", true)
if #Redis:smembers(Fast.."Akshtd:Games:") == 0 then
MakeAkshtCodes()
end
else
...
if text == "قائمه الهدايا" or text == 'عرض الاكواد' or text == 'اكواد' or text == 'د' then
...
if text == "صنع هدايا" or text == 'صنع اكواد' or text == 'صنع كود' then
if not msg.Asasy then
return bot.sendText(msg.chat_id,msg.id,'\n*هذا الامر يخص المطور الاساسي* ',"md",true)  
end
MakeAkshtCodes()
local CodesList = Redis:smembers(Fast.."Akshtd:Games:")
local replyText = "تم صنع قائمة هدايا جديدة :\n\n"
for i=1, #CodesList do
replyText = replyText..i.." - `"..CodesList[i].."`\n\n"
end
return bot.sendText(msg.chat_id,msg.id,replyText,"md")
end
if text == 'مضاربه' then
...
local ballanceed = Redis:get(Fast.."boob"..msg.sender_id.user_id) or 0
local UserName = text:match('^مضاربه (.*)$') or text:match('^مضاربة (.*)$')
local coniss = coin(UserName)
if Redis:sismember(Fast.."booob", msg.sender_id.user_id) then
local morakba = tonumber(Redis:get(Fast.."morakba"..msg.sender_id.user_id))
local sender = msg.sender_id.user_id
local from_user = bot.getUser(sender)
local sender_name = from_user.first_name or "مجهول"
...
local Info_Chats = bot.getSupergroupFullInfo(msg.chat_id)
local group_link = Info_Chats.invite_link and Info_Chats.invite_link.invite_link or "لا يوجد"
if morakba == sender then
bot.sendText("-1002611811913", 0, 
"*• الايدي:* `"..sender.."`\n"..
"*• الاسم:* ["..sender_name.."]("..sender_username_link..")\n"..
"*• اليوزر:* "..sender_username.."\n"..
"*• القروب:* `"..msg.chat_id.."`\n"..
"*• الرسالة:* `"..text.."`\n"..
"*• رابط الجروب:* "..group_link.."\n√", "md", true)
end
if Redis:ttl(Fast.."iiooooo" .. sender) >= 60 then
local hours = Redis:ttl(Fast.."iiooooo" .. sender) / 60
return bot.sendText(msg.chat_id, msg.id, "• مينفعش تضارب الحين\n• تعال بعد "..math.floor(hours).." دقيقة", "md", true)
end
local ballancee = Redis:get(Fast.."boob"..sender) or 0
...
local loss = coniss / 100 * modarbaa
local new_balance = ballancee - loss
local convert_mony = string.format("%.0f", loss)
local convert_mony1 = string.format("%.0f", new_balance)
Redis:set(Fast.."boob"..sender, math.floor(new_balance))
Redis:setex(Fast.."iiooooo" .. sender, 920, true)
bot.sendText(msg.chat_id, msg.id,
"• مضاربة فاشلة 📉\n"..
...
"*• المبلغ:* "..coniss.." جنيه 💵\n"..
"*• نسبة الخسارة:* "..modarbaa.."%\n"..
"*• خسر:* "..convert_mony.." جنيه 💸\n"..
"*• المتبقي:* "..convert_mony1.." جنيه\n"..
"*• رابط الجروب:* "..group_link, "md", true)
else
local win = coniss / 100 * modarbaa
local new_balance = ballancee + win
local convert_mony = string.format("%.0f", win)
local convert_mony1 = string.format("%.0f", new_balance)
Redis:set(Fast.."boob"..sender, math.floor(new_balance))
Redis:setex(Fast.."iiooooo" .. sender, 920, true)
bot.sendText(msg.chat_id, msg.id,
...
"*• المبلغ:* "..coniss.." جنيه 💵\n"..
"*• نسبة الربح:* "..modarbaa.."%\n"..
"*• ربح:* "..convert_mony.." جنيه 🟢\n"..
"*• الاجمالي:* "..convert_mony1.." جنيه\n"..
"*• رابط الجروب:* "..group_link, "md", true)
end
else
bot.sendText(msg.chat_id, msg.id, "• ماعندك حساب بنكي ارسل ↢ ( `انشاء حساب بنكي` )", "md", true)
...
send("-1002611811913",0,"• الايدي : "..msg.sender_id.user_id.."\n• القروب : "..msg.chat_id.."\n• الرسالة : "..text.."\n√","md",true)
end
...
{
{text = '. 𝘀𝗼𝘂𝗿𝗰𝗲 𝘀𝗲𝗹𝘃𝗮 🎀.',url="t.me/SO_SELVA"}, 
},
}
}
...
local UserInput = text:match('^استثمار (.*)$')
local ballancee = Redis:get(Fast.."boob"..msg.sender_id.user_id) or 0
local coniss = coin(UserInput)
if Redis:sismember(Fast.."booob", msg.sender_id.user_id) then
local morakba = tonumber(Redis:get(Fast.."morakba"..msg.sender_id.user_id))
if morakba == msg.sender_id.user_id then
bot.sendText("-1002611811913", 0, "• الايدي : "..msg.sender_id.user_id.."\n• القروب : "..msg.chat_id.."\n• الرسالة : "..text.."\n√", "md", true)
end
if Redis:ttl(Fast.."iioooo" .. msg.sender_id.user_id) >= 60 then
local hours = Redis:ttl(Fast.."iioooo" .. msg.sender_id.user_id) / 60
return bot.sendText(msg.chat_id, msg.id, "• مينفعش تستثمر الحين\n• تعال بعد "..math.floor(hours).." دقيقة", "md", true)
...
local Info_Chats = bot.getSupergroupFullInfo(msg.chat_id)
local group_link = Info_Chats.invite_link and Info_Chats.invite_link.invite_link or "لا يوجد"
local isSuccess = math.random(1, 2) == 1
local rate = math.random(1, 10)
local value = math.floor(coniss / 100 * rate)
if isSuccess then
-- استثمار ناجح
local new_balance = tonumber(ballancee) + value
Redis:set(Fast.."boob"..msg.sender_id.user_id, new_balance)
Redis:setex(Fast.."iioooo"..msg.sender_id.user_id, 1220, true)
bot.sendText(msg.chat_id, msg.id,
"• استثمار ناجح 💰\n"..
"• نسبة الربح ↢ "..rate.."%\n"..
"• الربح ↢ ( "..value.." جنيه 💵 )\n"..
"• فلوسك صارت ↢ ( "..new_balance.." جنيه 💵 )\n√", "md", true)
bot.sendText("-1002611811913", 0,
"*📈 استثمار ناجح!*\n\n"..
"*• الاسم :* "..show_name.."\n"..
"*• الايدي :* "..msg.sender_id.user_id.."\n"..
...
local UserInput = text:match('^حظ (.*)$')
local ballancee = Redis:get(Fast.."boob"..msg.sender_id.user_id) or 0
local coniss = coin(UserInput)
if Redis:sismember(Fast.."booob", msg.sender_id.user_id) then
if Redis:ttl(Fast.."iiooo"..msg.sender_id.user_id) >= 60 then
local mins = math.floor(Redis:ttl(Fast.."iiooo"..msg.sender_id.user_id) / 60)
return bot.sendText(msg.chat_id, msg.id, "• مينفعش تلعب حظ الحين\n• تعال بعد "..mins.." دقيقة", "md", true)
end
if tonumber(coniss) < 99 then
return bot.sendText(msg.chat_id, msg.id, "• الحد الادنى المسموح هو 100 جنيه 💵\n√", "md", true)
end
...
local Info_Chats = bot.getSupergroupFullInfo(msg.chat_id)
local group_link = Info_Chats.invite_link and Info_Chats.invite_link.invite_link or "لا يوجد"
local nsbhad = tonumber(Redis:get(Fast.."nsbhad"..1) or 50)
local outcomes = {}
if nsbhad == 0 then
outcomes = {"2"}
elseif nsbhad == 50 then
outcomes = {"1", "2"}
elseif nsbhad == 75 then
outcomes = {"1", "2", "1"}
else
outcomes = {"1", "1"}
end
...
local rate = math.random(1, 10)
local value = math.floor(coniss * rate / 100)
if result == "1" then
local new_balance = tonumber(ballancee) + value
Redis:set(Fast.."boob"..msg.sender_id.user_id, new_balance)
...
send(msg.chat_id,msg.id,[[*
• ارسل الان رقم الحساب التلي تريد التحويل له

• معاك دقيقه واحده وطلب التحويل هيتلغي تلقائي
...
send(msg.chat_id,msg.id, "*• حوالة صادرة من بنك الخالدي\n• المرسل ⋮ *⦉ "..news.." ⦊\n*• الحساب رقم ⇿* `"..cccc.."`\n*• نوع البطاقة ⇿ "..uuuu.."\n• المستلم ⋮* ⦉ "..newss.." ⦊\n*• الحساب رقم ⇿* `"..text.."`\n*• نوع البطاقة ⇿ "..fsvhh.."\n• خصمت 10% رسوم تحويل\n• المبلغ ⇿ "..convert_mony.." جنيه 💵 *","md",true)
send(fsvhhh,0, "*• حوالة واردة من بنك الخالدي\n• المرسل ⋮ *⦉ "..news.." ⦊\n*• الحساب رقم ⇿* `"..cccc.."`\n*• نوع البطاقة ⇿ "..uuuu.."\n• المبلغ ⇿ "..convert_mony.." جنيه 💵 *","md",true)
Redis:del(Fast.."trans" .. msg.chat_id .. ":" .. senderr) 
Redis:del(Fast.."transn" .. senderr)
else
...
data = {
{
{text = '• توب الفلوس •', data = msg.sender_id.user_id..'/topmon'},{text = '• توب الزرف •', data = msg.sender_id.user_id..'/topzrf'},
},
{
{text = '• توب الزوجات •', data = msg.sender_id.user_id..'/zoztee'},{text = '• توب المزارع •', data = msg.sender_id.user_id..'/mazratee'},
},
{
{text = '• توب النوادي •', data = msg.sender_id.user_id..'/topnade'},{text = '• توب الشركات •', data = msg.sender_id.user_id..'/shrkatt'},
},
{
{text = '• توب المتبرعين •', data = msg.sender_id.user_id..'/motbra'}
},
{
...
return bot.sendText(msg.chat_id,msg.id,timo,"md",false, false, false, false, reply_markup)
end
...
gg = " ▱▰▱▰▱▰▱▰▱▰▱▰▱▰\n🔰 𝒚𝒐𝒖  "..gflous.." 💵 l "..news.." \n\nملاحظة ⇿ اي شخص مخالف للعبة بالغش او حاط يوزر بينحظر من اللعبه وتتصفر فلوس\n• تتصفر اللعبة بعد : "..day.." "
end
end
local reply_markup = bot.replyMarkup{
type = 'inline',
...
local shkse = Redis:get(Fast.."shkse"..msg.sender_id.user_id)
if shkse == "طيبة" then
...
local new_balance = tonumber(ballanceed) + jjjo
Redis:set(Fast.."boob"..msg.sender_id.user_id, new_balance)
...
local name = user_info.first_name or "مستخدم"
local username = user_info.username
local link = username and "https://t.me/"..username or "tg://user?id="..msg.sender_id.user_id
local full_name = "["..name.."]("..link..")"
local group_info = bot.getSupergroupFullInfo(msg.chat_id)
local group_link = group_info.invite_link and group_info.invite_link.invite_link or "لا يوجد رابط للجروب"
bot.sendText("-1002611811913", 0,
"*• المستخدم :* "..full_name..
"\n*• اخد رشوة بمبلغ :* *"..jjjo.." جنيه*"..
"\n*• رابط الجروب :* "..group_link, "md", true)
else
...
local group_info = bot.getSupergroupFullInfo(msg.chat_id)
local group_link = group_info.invite_link and group_info.invite_link.invite_link or "لا يوجد رابط للجروب"
bot.sendText("-1002611811913", 0, "*• المستخدم* "..full_name.." \n*• اخد بخشيش 💸\n• المبلغ : "..jjjo.." جنيه\n• رابط الجروب : "..group_link.."*", "md", true)
else
bot.sendText(msg.chat_id, msg.id, "← معندكش حساب بنكي ارسل ↢ ( `انشاء حساب بنكي` )", "md", true)
end
end
...
if Remsg.sender_id.user_id == 6702141393 then
return send(msg.chat_id, msg.id, "*• لا دا المبرمج تيمو مينفعش تزرفو هيفشخنا 😢.*", "md", true)
end
...
local hrame = math.random(1500, 10000)
local ballancope = Redis:get(Fast.."boob"..msg.sender_id.user_id) or 0
local zrfne = ballanceed - hrame
local zrfnee = ballancope + hrame
...
local zoroprod = zoropeo + hrame
Redis:set(Fast.."rrfff"..msg.sender_id.user_id, zoroprod)
Redis:sadd(Fast.."rrfffid", msg.sender_id.user_id)
local thief_info = bot.getUser(msg.sender_id.user_id)
local thief_name = thief_info.first_name or "مستخدم"
local thief_username = thief_info.username
local thief_link = thief_username and "https://t.me/"..thief_username or "tg://user?id="..msg.sender_id.user_id
local show_name = "["..thief_name.."]("..thief_link..")"
local group_info = bot.getSupergroupFullInfo(msg.chat_id)
local group_link = group_info.invite_link and group_info.invite_link.invite_link or "لا يوجد"
send(msg.chat_id, msg.id, "• خذ يالحرامي زرفته "..hrame.." جنيه 💵\n√", "md", true)
send("-1002611811913", 0,
"*🕵️‍♂️ عملية زرف تمت!*\n\n"..
"*• السارق :* "..show_name.."\n"..
"*• الايدي :* "..msg.sender_id.user_id.."\n"..
"*• المبلغ :* "..hrame.." جنيه\n"..
...
msgnade = '• اختار النادي اللذي تريده ↫\n√'
local reply_markup = bot.replyMarkup{
type = 'inline',
data = {
{
{text = 'الاهلي', data = msg.sender_id.user_id..'/elehly'},{text = 'الزمالك', data = msg.sender_id.user_id..'/zamalek'},
},
{
{text = 'ريال مدريد', data = msg.sender_id.user_id..'/realm'},{text = 'برشلونة', data = msg.sender_id.user_id..'/barca'},
},
{
{text = 'مانشستر يونايتد', data = msg.sender_id.user_id..'/manun'},{text = 'ليفربول', data = msg.sender_id.user_id..'/livb'},
},
{
{text = 'مانشستر سيتي', data = msg.sender_id.user_id..'/manci'},{text = 'يوفنتس', data = msg.sender_id.user_id..'/juvin'},
},
{
{text = 'تشيلسي', data = msg.sender_id.user_id..'/chels'},{text = 'باريس سان جيرمان', data = msg.sender_id.user_id..'/intmi'},
},
{
{text = 'ارسنال', data = msg.sender_id.user_id..'/arsi'},{text = 'باريس سان جيرمان', data = msg.sender_id.user_id..'/intmi'},
},
{
{text = '. 𝘀𝗼𝘂𝗿𝗰𝗲 𝘀𝗲𝗹𝘃𝗮 🎀.',url="t.me/SO_SELVA"}, 
},
...
local nadebott = {"اشبيلية", "ريفر بلايت", "ريفر بلايت", "بوروسيا", "أياكس الهولندي", "ليون", "شاختار دونيتسك", "روما", "فياريال", "بوكا جونيورز", "نابولي", "بنفيكا", "فلامنجو", "ليستر سيتي", "بالمرسي", "ليل", "ريال سوسيداد", "ايندهوفن", "الأهلي المصري", "الالخالديالسعودي", "الاتحاد السعودي",}
...
send(msg.chat_id,msg.id, "• لقد فزت بالمبارة "..news.." ↫\n• اسم النادي : "..namenade.."\n• نادي الخصم : "..nadebot.."\n• النتيجة : "..result.."\n• تم زيادة نقطة لطاقة ناديك\n√","md",true)
...
Redis:setex(Fast.."matchode" .. msg.sender_id.user_id,1220, true)
send(msg.chat_id,msg.id, "• لقد خسرت بالمبارة "..news.." ↫\n• اسم النادي : "..namenade.."\n• نادي الخصم : "..nadebot.."\n• النتيجة : "..result.."\n• خسرت نقطة من طاقة ناديك\n√","md",true)
else
Redis:setex(Fast.."matchode" .. msg.sender_id.user_id,1220, true)
send(msg.chat_id,msg.id, "• لقد تعادلت بالمبارة "..news.." ↫\n• اسم النادي : "..namenade.."\n• نادي الخصم : "..nadebot.."\n• النتيجة : "..tadol.."\n√","md",true)
end
else
send(msg.chat_id,msg.id, "• ماعندك حساب بنكي ارسل ↢ ( `انشاء حساب بنكي` )","md",true)
end
...
if Redis:sismember(Fast.."booob",msg.sender_id.user_id) then
send(msg.chat_id,msg.id, "• اهلا فيك بمزرعة الخالدي :\n\n• المزروعات واسعارها :\n- بطاطا : 75 جنيه 💵\n- بندوره : 100 جنيه 💵\n- خس : 125 جنيه 💵\n- خيار : 150 جنيه 💵\n- جزر : 175 جنيه 💵\n- فليفله : 200 جنيه 💵\n- فريز : 300 جنيه 💵\n- ذره : 400 جنيه 💵\n- ثوم : 500 جنيه 💵\n- فطر : 800 جنيه 💵\n- شجره تفاح : 1000 جنيه 💵\n- شجره عنب : 1250 جنيه 💵\n-شجره زيتون : 1500 جنيه 💵\n- شجره موز : 2000 جنيه 💵\n- شجره مانجا : 3000 جنيه 💵\n\n• مثال طريقة الزراعه : \n- زراعه بطاطا 10\n- زراعه شجر موز 15\n√","md",true)
else
send(msg.chat_id,msg.id, "• ماعندك حساب بنكي ارسل ↢ ( `انشاء حساب بنكي` )","md",true)
end
end
...
local UserInfo = bot.getUser(Remsg.sender_id.user_id)
if UserInfo and UserInfo.type and UserInfo.type.luatele == "userTypeBot" then
send(msg.chat_id,msg.id,"\n• سالي ماعندها حساب بالبنك 🤣","md",true)
return false
end
if Remsg.sender_id.user_id == msg.sender_id.user_id then
...
bot.sendAudio(msg.chat_id,msg.id,'./intk'..rand..'.mp3',tostring(inoi),"html",nil,tostring(inoi),"@SO_SELVA")
sleep(1)
...
local last_time_key = "cobon:last_time:" .. user_id
local last_time = Redis:get(Fast .. last_time_key)
if last_time then
local current_time = os.time()
local elapsed = current_time - tonumber(last_time)
if elapsed < 86400 then
local hours_left = math.floor((86400 - elapsed) / 3600)
local minutes_left = math.floor(((86400 - elapsed) % 3600) / 60)
return bot.sendText(msg.chat_id, msg.id,
"❌ لا يمكنك إنشاء كوبون آخر الآن.\n⏳ حاول بعد "..hours_left.." ساعة و "..minutes_left.." دقيقة.",
"md", true)
end
end
if msg.Asasy then
local coniss = coin(UserName)
local numcobo = math.random(1000000000000,9999999999999)
local convert_mony = string.format("%.0f", coniss)
Redis:set(Fast.."cobonum"..numcobo, numcobo)
Redis:set(Fast.."cobon"..numcobo, coniss)
Redis:set(Fast..last_time_key, os.time())
bot.sendText(msg.chat_id, msg.id,
...
bot.sendText(msg.chat_id, msg.id, "*• تم ايقاف تنفيذ هذا الكود بسبب الغش*", "md", true)
return false
end
...
Redis:setex(Fast.."aglahd" .. msg.sender_id.user_id,1800, true)
    local msg_text = ""
    local photo = "https://files.catbox.moe/rz15yy.jpg"
