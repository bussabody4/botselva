...
if text == "رفع بقلبي" or text == "رفع في قلبي" or text == "رفع قلبي" then
local Message_Reply = bot.getMessage(msg.chat_id, msg.reply_to_message_id)
...
if text == "انا فقلب مين" or text == "انا في قلب مين" or text == "مين قلبي" or text == "قلبي مين" then
...
local photo = bot.getUserProfilePhotos(in_heart_id)
local UserInfo = bot.getUser(in_heart_id)
if UserInfo.username then
UserName = ' '..UserInfo.username..''
else
UserName = 'Sudanese_56' 
end
local reply_markup = bot.replyMarkup{type = 'inline',data = {
{
{text = heart_name, url = 't.me/'..UserInfo.username..' '},
},
}
}
bot.sendPhoto(msg.chat_id, msg.id, photo.photos[1].sizes[#photo.photos[1].sizes].photo.remote.id,"*💗 انت فقلب ⤶* ["..heart_name.."](tg://user?id="..in_heart_id..") ","md", true, nil, nil, nil, nil, nil, nil, nil, nil, reply_markup )
end
end
...
if text == "مسح كل القلوب" then
if not msg.Admin then
return send(msg_chat_id,msg_id,'* • انا لست ادمن\n√*',"md",true)  
end
local all_keys = Redis:keys(Fast..msg_chat_id.."*")
for _, key in ipairs(all_keys) do
if key:match("in_heart:") or key:match("my_heart:") then
Redis:del(key)
end
end
return send(msg_chat_id, msg_id, "*• تم مسح كل القلوب من الجميع 💔*","md")
end
-------
-- ttgwzine
...
local nnbot = (Redis:get(Fast.."Name:Bot") or "سيلفا")
...
local reply_markup = bot.replyMarkup{type = 'inline',data = {
{
{text = zwga_name, url = 't.me/'..UserInfo.username..' '},
},
}
}
bot.sendPhoto(msg.chat_id, msg.id, photo.photos[1].sizes[#photo.photos[1].sizes].photo.remote.id,Text,"md", true, nil, nil, nil, nil, nil, nil, nil, nil, reply_markup )
end
