...
{
{text = 'الغاء الحـظـر ❇️', callback_data=senderr..'unban'..rep_idd},
},
}
local Text = Reply_Status(UserId_Info.id,"• تم حظره من الجروب ").Reply
local Timo = Reply_Status(UserId_Info.id,"• تم حظره من الجروب مسبقا ").Reply
if Redis:sismember(Fast.."BanGroup:Group"..msg_chat_id,UserId_Info.id) then
local msgg = msg_id/2097152/0.5
https.request("https://api.telegram.org/bot"..Token.."/sendvideo?chat_id=" .. msg_chat_id .. "&video=https://t.me/UOUSOU/56&caption=".. URL.escape(Timo).."&photo="..msgg.."&parse_mode=markdown&disable_web_page_preview=true&reply_markup="..JSON.encode(keyboard))
else
Redis:sadd(Fast.."BanGroup:Group"..msg_chat_id,UserId_Info.id) 
bot.setChatMemberStatus(msg.chat_id,rep_idd,'banned',0)
local msgg = msg_id/2097152/0.5
https.request("https://api.telegram.org/bot"..Token.."/sendvideo?chat_id=" .. msg_chat_id .. "&video=https://t.me/UOUSOU/56&caption=".. URL.escape(Text).."&photo="..msgg.."&parse_mode=markdown&disable_web_page_preview=true&reply_markup="..JSON.encode(keyboard))
end
end 
...
keyboard.inline_keyboard = {
{
{text = 'الـغـاء الـكـتـم 📢', callback_data=senderr..'unmute'..rep_idd},
},
}
local Text = Reply_Status(UserId_Info.id, "• تم كتمه في الجروب ").Reply
local Timo = Reply_Status(UserId_Info.id, "• تم كتمه في الجروب مسبقًا ").Reply
if Redis:sismember(Fast.."SilentGroup:Group"..msg_chat_id,UserId_Info.id) then
local msgg = msg_id/2097152/0.5
https.request("https://api.telegram.org/bot"..Token.."/sendvideo?chat_id=" .. msg_chat_id .. "&video=https://t.me/UOUSOU/58&caption=".. URL.escape(Timo).."&photo="..msgg.."&parse_mode=markdown&disable_web_page_preview=true&reply_markup="..JSON.encode(keyboard))
else
Redis:sadd(Fast.."SilentGroup:Group"..msg_chat_id,UserId_Info.id) 
local msgg = msg_id/2097152/0.5
https.request("https://api.telegram.org/bot"..Token.."/sendvideo?chat_id=" .. msg_chat_id .. "&video=https://t.me/UOUSOU/58&caption=".. URL.escape(Text).."&photo="..msgg.."&parse_mode=markdown&disable_web_page_preview=true&reply_markup="..JSON.encode(keyboard))
end
end
...
local Text = Reply_Status(rep_idd,"• تم حظره من الجروب ").Reply
local Timo = Reply_Status(rep_idd,"• تم حظره من الجروب مسبقا ").Reply
...
bot.sendVideo(msg_chat_id, msg_id, "https://t.me/UOUSOU/55", "• تم كتم القناة من المجموعة")
return
end
local UserInfo = bot.getUser(rep_idd)
if UserInfo.message == "Invalid user ID" then
...
local Text = Reply_Status(rep_idd, "• تم كتمه في الجروب ").Reply
local Timo = Reply_Status(rep_idd, "• تم كتمه في الجروب مسبقًا ").Reply
...
keyboard.inline_keyboard = {
{
{text = 'الغاء التقييد ❇️', callback_data=senderr..'tqyeeed'..rep_idd},
},
}
local Text = Reply_Status(rep_idd,"• تم تقييده في الجروب ").Reply
bot.setChatMemberStatus(msg.chat_id,rep_idd,'restricted',{1,0,0,0,0,0,0,0,0})
local msgg = msg_id/2097152/0.5
https.request("https://api.telegram.org/bot"..Token.."/sendvideo?chat_id=" .. msg_chat_id .. "&video=https://t.me/UOUSOU/57&caption=".. URL.escape(Text).."&photo="..msgg.."&parse_mode=markdown&disable_web_page_preview=true&reply_markup="..JSON.encode(keyboard))
end
...
local Text = Reply_Status(UserId,"• تم حظره من الجروب ").Reply
local Timo = Reply_Status(UserId,"• تم حظره من الجروب مسبقا ").Reply
...
local Text = Reply_Status(UserId, "• تم كتمه في الجروب ").Reply
local Timo = Reply_Status(rep_idd, "• تم كتمه في الجروب مسبقًا ").Reply
...
Get_Json = Get_Json..'"Dev":"Y_a_z"}'
end
Get_Json = Get_Json..'}}'
local File = io.open('./'..UserBot..'.json', "w")
File:write(Get_Json)
File:close()
return bot.sendDocument(msg_chat_id,msg_id,'./'..UserBot..'.json', '*╖ تم جلب النسخه الاحتياطيه عزيزي المطور\n╢ تحتوي على ❴ '..#Groups..' ❵ جروب \n╜ وتحتوي على ❴ '..#UsersBot..' ❵ مشترك \n√*','md')
end
if text == '• تفعيل جلب النسخه التلقائي •' then   
if not msg.Devss then
return send(msg_chat_id,msg_id,'\n*• هذا الامر يخص { '..Controller_Num(2)..' }* ',"md",true)  
end
...
if text == '• تعطيل جلب النسخه التلقائي •' then   
if not msg.Devss then
return send(msg_chat_id,msg_id,'\n*• هذا الامر يخص { '..Controller_Num(2)..' }* ',"md",true)  
end
...
File:write(Get_Json)
File:close()
bot.sendDocument(Sudo_Id,0,'./'..UserBot..'.json', '*╖ تم جلب النسخه الاحتياطيه عزيزي المطور\n╢ تحتوي على ❴ '..#Groups..' ❵ جروب \n╜ وتحتوي على ❴ '..#UsersBot..' ❵ مشترك \n√*','md')
local Get_Json = '{"BotId": '..Fast..','  
Get_Json = Get_Json..'"GroupsBotreply":{'
...
local Selvaa = bot.getUser(msg.sender_id.user_id)
local Timo = "*اليك اسمك عزيزي "..msg.Name_Controller.." 🥺❤️*"
local reply_markup = bot.replyMarkup{type = 'inline',data = {
{
{text = Selvaa.first_name, url = "https://t.me/"..Selvaa.username..""},
},
} 
}
...
send(msg_chat_id, msg_id, '• البايو : ['..FlterBio(Bio)..']', 'md',false, false, false, false, reply_markup)
end
...
data = {
{
{text = '• نعم •', data = senderr..'/Nzlne'},
},
{
{text = '• لا •', data = senderr..'/noNzlne'},
},
}
}
return send(msg_chat_id,msg_id,'*• هل انت متاكد من تنزيلك من جميع رتب البوت*',"md",false, false, false, false, reply_markup)
end
