...
if text == "تفعيل اذكار" or text == "تفعيل الاذكار" then
...
local Timo = "*• تم تفعيل الاذكار \n • بواسطه ⦉ "..Text.." ⦊ *"
Redis:set(Fast..'az_kaar'..msg.chat_id,true)
send(msg.chat_id,msg.id,Timo,"md",true)
end
if text == "تعطيل اذكار" or text == "تعطيل الاذكار" then
if not msg.Manger then
return send(msg.chat_id,msg.id,'\n*•  هذا الامر يخص '..Controller_Num(6)..' * ',"md",true)
end
local ban = bot.getUser(Fast)
...
local Timo = "*• تم تعطيل الاذكار \n • بواسطه ⦉ "..Text.." ⦊ *"
Redis:del(Fast..'az_kaar'..msg.chat_id)
send(msg.chat_id,msg.id,Timo,"md",true)
end

if Redis:get(Fast.."az_kaar"..msg_chat_id) then 
